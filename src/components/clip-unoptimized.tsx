...
import useClipUnoptimized from "../hooks/use-clip-unoptimized";
import useBvh from "../hooks/use-bvh";
import { PivotControls, Segments, Wireframe } from "@react-three/drei";

const ClipUnoptimized = () => {
  const theLongestEdgeLength = useAtomValue(theLongestEdgeLengthAtom);

  const clipPlaneMeshRef = useRef<Mesh>(null);

  const { clip } = useClipUnoptimized();
...
export default ClipUnoptimized;
